...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (left <= last) {
            if (left < last && order.compare(array[right], array[left]) < 0) {
                smallest = right;
            }
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        int leftOfLeft = 2 * left + 1;
        int rightOfLeft = 2 * left + 2;
        int smallest = leftOfLeft;

        if (leftOfLeft <= array.length - 1) {
            if (leftOfLeft < array.length - 1 && order
                    .compare(array[rightOfLeft], array[leftOfLeft]) < 0) {
                smallest = rightOfLeft;
            }
            if (order.compare(array[left], array[smallest]) > 0) {
                exchangeEntries(array, left, smallest);
                heapify(array, left, order);
            }
        }

        //Heapify the right subtree
        int leftOfRight = 2 * right + 1;
        int rightOfRight = 2 * right + 2;
        smallest = rightOfRight;

        if (leftOfRight <= array.length - 1) {
            if (leftOfRight < array.length - 1 && order
                    .compare(array[rightOfRight], array[leftOfRight]) < 0) {
                smallest = rightOfRight;
            }
            if (order.compare(array[right], array[smallest]) > 0) {
                exchangeEntries(array, right, smallest);
    ...
    public final T removeFirst() {
        ...
        //Takes the top and swaps it with the bottom element. The top is then
        //sifted down
