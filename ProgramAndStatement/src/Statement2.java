...
public class Statement2 extends StatementSecondary {
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        //disassemble tree for the sequence of children and the root.
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        StatementLabel root = this.rep.disassemble(children);

        //set a new Statement2 to represent s.
        Statement2 sDup = (Statement2) s;

        //add s to this, and clear the new Statement2
        children.add(pos, sDup.rep);
        this.rep.assemble(root, children);
        sDup.createNewRep();
    ...
    public final Statement removeFromBlock(int pos) {
        ...
        //disassemble the tree to get the sequence of children and the root.
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        StatementLabel root = this.rep.disassemble(children);
        //remove the StatementLabel at the specific position.
    ...
    public final int lengthOfBlock() {
        ...
        //disassemble the tree for the sequence of children and the root.
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        StatementLabel root = this.rep.disassemble(children);
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        //create a new StatementLabel with type IF_ELSE.
        StatementLabel label = new StatementLabel(Kind.IF_ELSE, c);

        //create a sequence in order to add the statements in it.
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();

        //add the 2 statements into the sequence and assemble the tree.
        children.add(0, localSIf.rep);
        children.add(1, localSElse.rep);
        this.rep.assemble(label, children);

        //clear the local variables.
        localSIf.createNewRep();
        localSElse.createNewRep();
    }
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        //remove the stements from the sequence.
    ...
    public final void assembleWhile(Condition c, Statement s) {
        ...
        //create a new StatementLabel with type WHILE.
        StatementLabel label = new StatementLabel(Kind.WHILE, c);

        //create a sequence in order to add the statements in it.
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();

        //add the statement into the sequence and assemble the tree.
        children.add(0, localS.rep);
        this.rep.assemble(label, children);
    ...
    public final void assembleCall(String inst) {
        ...
        //create a new StatementLabel with type CALL.
