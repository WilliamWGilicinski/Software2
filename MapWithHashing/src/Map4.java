 ...
 * </pre>
 *
 * @author William Gilicinski, Colton Davie
 *
 */
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final void add(K key, V value) {
        ...
        //Creates the map 2 object to put into the hashtable
        Map2<K, V> temp = new Map2<>();
        temp.add(key, value);
        //Finds out which bucket to put the Map2 in
        int hashCode = key.hashCode();
        int bucket = mod(hashCode, this.hashTable.length);
        this.hashTable[bucket] = temp;
    ...
    public final Pair<K, V> remove(K key) {
        ...
        //Finds bucket where key is located and removes it from Map
        int hashCode = key.hashCode();
        int bucket = mod(hashCode, this.hashTable.length);
        Pair<K, V> deleted = this.hashTable[bucket].remove(key);
    ...
    public final Pair<K, V> removeAny() {
        ...
        //Create an iterator
        Iterator<Pair<K, V>> iterate = new Map4Iterator();
        int lowestHashCode = Integer.MAX_VALUE;

        //Iterate to find lowest hashcode key
        while (iterate.hasNext()) {
            int currentHashCode = iterate.next().key().hashCode();
            if (currentHashCode < lowestHashCode) {
                lowestHashCode = currentHashCode;
            }
        }

        //Find out the bucket the lowest hashcode was in
        int bucket = mod(lowestHashCode, this.hashTable.length);
    ...
    public final V value(K key) {
        ...
        int hashCode = key.hashCode();
        int bucket = mod(hashCode, this.hashTable.length);
        V info = this.hashTable[bucket].value(key);
